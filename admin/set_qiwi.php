...
$row1 = DB::$the->query("SELECT * FROM `sel_set_bot` ");
$set1 = $row1->fetch(PDO::FETCH_ASSOC);
...
if(isset($_GET['resetprofitqiwi'])) {
    DB::$the->prepare("UPDATE sel_set_bot SET profit_qiwi='0'")->execute();
}
?>
    ...
    <form method="POST" action="?ok"><div class="form-group col-sm-8">
            ...
            <div class="input-group input-group-lg">
                ...
                <input type="text" class="form-control" name="limits" value="<?=$set1['limits'];?>">
                <span class="input-group-addon">руб.</span>
            </div><br />
            <div class="input-group input-group-lg">
                <span class="input-group-addon">Оборот денег за всё время</span>
                <input type="text" class="form-control" value="<?=$profit['profit_qiwi']?> руб"disabled>
            </div>
            <br><br>
